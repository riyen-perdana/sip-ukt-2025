...
use App\Livewire\Forms\LoginMahasiswaForm;
...
{
    public LoginMahasiswaForm $form;
...
<div>
    ...
    <form wire:submit="login">
        ...
        <!-- Email Address -->
        <div>
            <x-input-label class="text-xs font-semibold" for="nim" :value="__('Username [Sesuai iRaise]*')" />
            <x-text-input wire:model="form.nim" id="nim" class="block w-full mt-1" type="text" name="nim" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('form.nim')" class="mt-2" />
        </div>

        <!-- Password -->
        <div class="mt-4">
            <x-input-label class="text-xs font-semibold" for="password" :value="__('Password [Sesuai iRaise]*')" />
        ...
        <div class="block mt-4">
            <label for="remember" class="inline-flex items-center">
                ...
                <span class="text-xs text-gray-600 ms-2">{{ __('Ingat Saya') }}</span>
            </label>
        </div>
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <x-link-button class="bg-sky-500 ms-1 hover:bg-sky-400 focus:bg-sky-400 active:bg-sky-400" :href="route('welcome')" wire:navigate>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h-3.5 me-2">
