                        ...
                        <div class="flex flex-row text-xs gap-y-2">
                            <table class="table w-full leading-normal border-none">
                                @foreach ($data as $i => $item)
                                    @if ($item->pengajuan->count() == 0 && $item->is_aktif == 'N')
                                        @continue
                                    @else
                                        @php
                                            $pengajuan = $item->pengajuan->first() ?? null;
                                            $verifikasi = $pengajuan ? $pengajuan->verifikasi->first() : null;
                                            // $verifikasi = $pengajuan->verifikasi->first() ?? null;
                                        @endphp
                                        ...
                                        <tr>
                                            ...
                                            <td class="font-semibold text-right">
                                                @if ($verifikasi && $verifikasi->is_setuju == 'Y')
                                                    <span class="text-green-500">{{ __('Diterima') }}</span>
                                        ...
                                        <tr>
                                            ...
                                            <td class="pt-2 font-semibold text-right">
                                                @if ($item->daftar_buka <= $date && $item->daftar_tutup >= $date && !$pengajuan)
