    ...
    <x-modal name='beasiswa-modal'>
        ...
        <form class="px-6 pt-6 pb-4 mb-4 bg-white rounded" wire:submit="save" wire:loading.attr="disabled">
            ...
            <div class="grid grid-cols-1 gap-4 mb-2 sm:grid-cols-2">
                <div>
                    <x-input-label for="nim" :value="__('Nomor Induk Mahasiswa')" class="mb-2 font-semibold" />
                    <x-text-input wire:model="nim" id="nim"
                        class="w-full px-3 py-2 text-sm leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
                        type="text" name="nim" required placeholder="Nomor Induk Mahasiswa" />
                    <x-input-error :messages="$errors->get('nim')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="name" :value="__('Nama')" class="mb-2 font-semibold" />
                    <x-text-input wire:model="name" id="name"
                        class="w-full px-3 py-2 text-sm leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
                        type="text" name="name" required placeholder="Nama" />
