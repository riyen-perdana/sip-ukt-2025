    ...
    <x-modal name='pendaftar-modal'>
        ...
        <section class="px-6 pt-6 pb-4 mb-4 data">
            <div class="flex flex-col gap-2">
                <x-button-table x-data=""
                    x-on:click.prevent="$dispatch('showModal',{view:'{{ $surat_permohonan }}'})"
                    wire:loading.attr="disabled" class="flex flex-row bg-cyan-500 hover:bg-cyan-600">
        ...
        <section class="form_data">
            <form class="px-6 pt-0 pb-4 mb-4 bg-white rounded" wire:submit="save" wire:loading.attr="disabled">
                ...
                <div class="mb-2">
                    ...
                    <select wire:model.live="is_setuju"
                        class="w-full px-3 py-2 text-sm leading-tight text-gray-700 border border-gray-300 rounded shadow appearance-none focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none focus:shadow-outline">
                ...
                @if ($isShowUkt)
                    <div class="mb-2">
                        <x-input-label for="ukt_pilihan" :value="__('Besaran UKT')" class="mb-2 font-semibold" />
                        <select wire:model="ukt_pilihan"
                            class="w-full px-3 py-2 text-sm leading-tight text-gray-700 border border-gray-300 rounded shadow appearance-none focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none focus:shadow-outline">
                            <option value="">{{ __('Pilih Besaran UKT') }}</option>
                            @foreach ($ukt as $key => $option)
                                <option value="{{ $key }}-{{ $option }}">UKT {{ $key }} - {{ $option }}
                                </option>
                            @endforeach
                        </select>
                        <x-input-error :messages="$errors->get('ukt_pilihan')" class="mt-2" />
                    </div>
                @endif
                <div class="mb-2">
                    <x-input-label for="komentar" :value="__('Catatan Persetujuan')" class="mb-2 font-semibold" />
                    <textarea wire:model="komentar"
                        class="w-full px-3 py-2 text-xs leading-tight text-gray-700 border border-gray-300 rounded shadow appearance-none focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none focus:shadow-outline"></textarea>
