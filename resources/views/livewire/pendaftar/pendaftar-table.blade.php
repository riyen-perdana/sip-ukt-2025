                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <th scope="row"
                            ...
                            <section class="flex-shrink-0 foto">
                                ...
                                @endphp
                                <img src="{{ $url }}" alt="" class="w-20 h-20 rounded-full">
                            </section>
                            <section class="flex flex-col data">
                                ...
                                @if ($item->pengajuan[$i]->status == 'ajk')
                                    <div>
                                        Status : <span class="font-semibold text-red-500">Pengajuan</span>
                                    </div>
                                @elseif ($item->pengajuan[$i]->status == 'prs')
                                    <div>
                                ...
                                @php
                                    $verifikasi = $item->pengajuan[0]['verifikasi'][0] ?? null;
                                @endphp
                                ...
                                @elseif ($verifikasi && $verifikasi->is_setuju == 'N')
                                    <div>
                                        Pengajuan : <span class="font-semibold text-red-500">Ditolak</span>
                                    </div>
                                @else
                                    <div>
                                        Pengajuan : <span class="font-semibold">Belum Disetujui</span>
                                    </div>
                                @endif
                            </section>
                        </th>
                        <th scope="row"
                            class="px-3 py-3 font-normal text-center align-top text-[13px] text-gray-900 whitespace-nowrap">
                            <div>UKT Awal : <span class="font-semibold text-red-500">[UKT {{ $item->ukt }}] -
                                    {{ format_rupiah($item->jml_ukt_awal) }}</span>
                            </div>
                            @if ($verifikasi && $verifikasi->is_setuju == 'Y' && $verifikasi->kelompok_ukt != '0')
                                <div>Kelompok UKT Final : <span class="font-semibold text-green-500">UKT - {{ $verifikasi->kelompok_ukt }}</span></div>
                                <div>Besaran UKT Final : <span class="font-semibold text-green-500">{{ format_rupiah($verifikasi->besaran_ukt) }}</span></div>
                            @else
                                <div>Kelompok UKT Final : <span class="font-semibold text-green-500">TETAP</span></div>
                                <div>Besaran UKT Final : <span class="font-semibold text-green-500">TETAP</span></div>
                            @endif
                        </th>
                        <th scope="row"
                            class="px-3 py-3 font-normal align-top text-[13px] text-gray-900 whitespace-nowrap text-center">
                            @if ($verifikasi && $verifikasi->user)
