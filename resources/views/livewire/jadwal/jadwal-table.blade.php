<div wire:poll class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <section class="flex flex-row items-center justify-between mx-auto mt-5 mb-5 search-pagination">
        <div class="w-5/12 ml-4">
            <x-text-input wire:model.live="search" id="search"
                ...
                type="text" placeholder="Cari Data Tahun" />
        </div>
        ...
        <tbody>
            @forelse($data as $item)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <th scope="row"
                        class="flex flex-col px-3 py-3 text-[13px] font-normal text-center text-gray-900 whitespace-nowrap">
                        <div>{{ $item->tahun }}</div>
                    </th>
                    <th scope="row"
                        class="px-3 py-3 text-[13px] font-normal text-center text-gray-900 whitespace-nowrap">
                        {{ tanggal_indonesia($item->daftar_buka,false) }}
                    </th>
                    <th scope="row"
                        class="px-3 py-3 text-[13px] font-normal text-center text-gray-900 whitespace-nowrap">
                    ...
                    <th scope="row">
                        <div class="flex flex-row justify-center gap-x-1">
                            ...
                            <x-button-table
                                ...
                                wire:click="$dispatch('sweet-alert-confirm',{id:'{{ $item->id }}',title:'Hapus Tahun {{ $item->tahun }}'})"
